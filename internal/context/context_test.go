...
func TestManager_CopyContextFiles_GlobPattern(t *testing.T) {
	...
	docsDir := filepath.Join(projectRoot, "docs")
	_ = os.MkdirAll(docsDir, 0750)
	_ = os.WriteFile(filepath.Join(docsDir, "file1.md"), []byte("# File 1"), 0600)
	_ = os.WriteFile(filepath.Join(docsDir, "file2.md"), []byte("# File 2"), 0600)
	_ = os.WriteFile(filepath.Join(docsDir, "file3.txt"), []byte("Not markdown"), 0600)
...
func TestManager_CopyContextFiles_MixedPatterns(t *testing.T) {
	...
	docsDir := filepath.Join(projectRoot, "docs")
	_ = os.MkdirAll(docsDir, 0750)
	_ = os.WriteFile(filepath.Join(docsDir, "exists.md"), []byte("# Exists"), 0600)
...
func TestManager_ListContextFiles(t *testing.T) {
	...
	_ = os.MkdirAll(contextDir, 0750)
	_ = os.WriteFile(filepath.Join(contextDir, "file1.md"), []byte("# File 1"), 0600)
	_ = os.WriteFile(filepath.Join(contextDir, "file2.md"), []byte("# File 2"), 0600)
